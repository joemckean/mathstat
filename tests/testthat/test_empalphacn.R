...
test_that("input", {
	expect_error(empalphacn(NaN),
	             " * argument 1 cannot include a NaN")
	expect_error(empalphacn(Inf),
	             " * argument 1 cannot include an Inf or -Inf")
  expect_error(empalphacn("a"),
...
test_that("output", {
  set.seed(1234)
  samp <- empalphacn(10)
  expect_true(samp$empiricalalpha >= .1-ep && samp$empiricalalpha <= .1+ep)
...
test_that("limits", {
  ...
  expect_error(empalphacn(.5),
	             " * argument 1 must be an integer")
  expect_error(empalphacn(0),
	             " * argument 1 must be numeric and non-zero")
  expect_error(empalphacn(-1),
