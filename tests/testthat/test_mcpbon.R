...
test_that("edge cases", {
    expect_error(mcpbon(dummy_inf, car),
                 "argument 1 cannot include an Inf or -Inf")
    expect_error(mcpbon(speed, car, alpha = 1.1),
                 "input object 'alpha' must be between 0 and 1")
    expect_error(mcpbon(speed, car, alpha = -1),
                 "argument 3 must be positive")
})

test_that("input", {
    expect_error(mcpbon(speed, car, alpha = NA),
...
test_that("output", {
    expect_equal(names(mcpbon(speed, car)),
                 c("j", "jp", "muj", "mujp", "diff", "se", "err", "lb", "ub"))
