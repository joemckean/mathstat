...
test_that("is_negative works", {
  ...
  expect_warning(is_negative(c(5, 4, 3.4), 1), "the condition has length > 1 and only the first element will be used")
})
...
test_that("is_nonzero works", {
  ...
  expect_warning(is_nonzero(c(5, 4, 3.4), 1), "the condition has length > 1 and only the first element will be used")
...
test_that("is_manyelement works", {
  expect_equal(is_manyelement(1, 1), "argument 1 must have length greater than 1")
  expect_equal(is_manyelement(c(1, 2, 3), 1), NULL)
  expect_equal(is_manyelement("xst", 1), "argument 1 must have length greater than 1")
...
test_that("is_inrange works", {
  ...
  expect_equal(samp, "argument 1 must be greater than or equal to 0 and less than or equal to 3")
  expect_equal(is_inrange(0, 1, 1, 3, "hey"), "hey")
})

test_that("is_xrange works", {
  # works in middle
  samp <- is_xrange(0.5, 1, 0, 1)
  expect_equal(samp, NULL)
  # works when x = min
...
test_that("has_noinf works", {
  ...
  expect_equal(has_noinf(-Inf, 1), "argument 1 cannot include an Inf or -Inf")
  expect_equal(has_noinf(c(1, 3, 2, Inf, 3), 1), "argument 1 cannot include an Inf or -Inf")
...
test_that("has_nonan works", {
  ...
  expect_equal(has_nonan(NaN, 1), "argument 1 cannot include a NaN")
  expect_equal(has_nonan(c(1, 3, 2, NaN, 3), 1), "argument 1 cannot include a NaN")
