...
test_that("edge cases", {
    expect_error(aresimcn(Inf),
                 "argument 1 cannot include an Inf or -Inf")
    expect_error(aresimcn(n = -1),
                 "argument 1 must be positive")
    expect_error(aresimcn(n = 50, nsims = Inf),
                 "argument 2 cannot include an Inf or -Inf")
    expect_error(aresimcn(n = 50, nsims = -1),
                 "argument 2 must be positive")
    expect_error(aresimcn(n = 50, eps = -1),
                 "input argument 'eps' must be between zero and one")
    expect_error(aresimcn(n = 50, eps = 1),
                 "input argument 'eps' must be between zero and one")
    expect_error(aresimcn(n = 50, nsims = 100, eps = Inf),
                 "argument 3 cannot include an Inf or -Inf")
    expect_error(aresimcn(n = 50, nsims = 100, vc = -1),
                 "argument 4 must be positive")
    expect_error(aresimcn(n = 50, vc = Inf),
...
test_that("input", {
    ...
    expect_error(aresimcn(n = 0),
                 "argument 1 must be numeric and non-zero")
    expect_error(aresimcn(n = NA),
    ...
    expect_error(aresimcn(nsims = NA),
    ...
    expect_error(aresimcn(n = 50, eps = NA),
                 "argument 3 must be a number")
    # Checking invalid input for vc
    expect_error(aresimcn(n = 50, vc = NA),
                 "argument 4 must be positive")
    expect_error(aresimcn(n = 50, vc = NA),
